...
function createContext(brand: CustomBrandDNA): GenerationContext {
  const accentRgb = hexToRgb(brand.accent);
  const shadow = brand.shadow === "sharp"
    ? `shadow-[0_9px_0_rgba(${hexToRgb(brand.ink)},0.22)]`
    : brand.shadow === "soft"
  ...
  return {
    ...
    focusRing: `focus-visible:ring-[rgba(${accentRgb},0.34)]`,
    ink: brand.ink,
    interaction: brand.material === "elastic"
      ? "duration-300 ease-out hover:-translate-y-0.5 hover:scale-[1.01]"
      : brand.material === "crisp"
        ? "duration-150 ease-out hover:-translate-y-px"
        : "duration-300 ease-out hover:-translate-y-px",
    interactionPressed: brand.material === "elastic" ? "active:translate-y-0 active:scale-[0.98]" : "active:translate-y-0",
    radius: brand.radius,
    shadow,
...
export function isGeneratedManifest(value: unknown): value is BrandGenerationManifest {
  ...
  return record.schemaVersion === 1 && Array.isArray(record.files) && record.files.length === GENERATED_COMPONENT_IDS.length && Boolean(record.brand?.slug);
}
