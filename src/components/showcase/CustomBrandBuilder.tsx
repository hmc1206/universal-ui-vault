...
import { CUSTOM_COMPONENT_IDS, CUSTOM_FONT_OPTIONS, CUSTOM_MATERIAL_OPTIONS, CUSTOM_PALETTE_OPTIONS, CUSTOM_RADIUS_OPTIONS, CUSTOM_SHADOW_OPTIONS, generateToneScale, getComponentOverride, isHexColor } from "./custom-brand";
...
export function CustomBrandBuilder({ brand, onChange, onReset, onSave, saved }: CustomBrandBuilderProps) {
  ...
  function patchTokens<Key extends keyof CustomSemanticTokens>(key: Key, value: CustomSemanticTokens[Key]) {
    ...
    const activeColorMode = brand.activeColorMode ?? "light";
    onChange({ ...brand, accent: key === "primary" ? value : brand.accent, colorModes: { ...(brand.colorModes ?? { dark: brand.tokens, light: brand.tokens }), [activeColorMode]: tokens }, ink: key === "ink" ? value : brand.ink, surface: key === "surface" ? value : brand.surface, toneScale: generateToneScale(tokens.primary), tokens });
  }

  function selectColorMode(mode: "light" | "dark") {
    const tokens = brand.colorModes?.[mode] ?? brand.tokens;
    onChange({ ...brand, accent: tokens.primary, activeColorMode: mode, ink: tokens.ink, surface: tokens.surface, toneScale: generateToneScale(tokens.primary), tokens });
  }
          ...
          {advancedOpen ? (
            <div className="grid gap-4">
              <TokenSection description="색상 값은 바로 ThemeBridge custom skin, 생성된 Tailwind source, live preview에 연결됩니다." title="Semantic palette · 직접 HEX 입력">
                <div className="sm:col-span-2 xl:col-span-3 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-[#e0dbed] bg-[#fbfaff] p-3"><div><p className="text-sm font-bold text-[#373143]">Color mode · {brand.activeColorMode === "dark" ? "Dark" : "Light"}</p><p className="mt-1 text-xs text-[#726b7d]">선택한 mode의 semantic tokens가 preview와 생성 소스에 적용됩니다.</p></div><div className="flex gap-2"><button className={joinClasses("min-h-9 rounded-lg border px-3 text-xs font-bold", brand.activeColorMode !== "dark" ? "border-[#6d55dc] bg-[#6d55dc] text-white" : "border-[#d3cce1] bg-white text-[#554d62]")} onClick={() => selectColorMode("light")} type="button">Light</button><button className={joinClasses("min-h-9 rounded-lg border px-3 text-xs font-bold", brand.activeColorMode === "dark" ? "border-[#24202e] bg-[#24202e] text-white" : "border-[#d3cce1] bg-white text-[#554d62]")} onClick={() => selectColorMode("dark")} type="button">Dark</button></div></div>
              ...
              <TokenSection description="primary에서 파생된 50~900 scale입니다. 생성 결과에는 active mode의 semantic token을 사용하며, scale은 설계 검토와 향후 theme export에 포함됩니다." title="Primary tone scale"><div className="sm:col-span-2 xl:col-span-3 grid grid-cols-5 gap-2">{Object.entries(brand.toneScale ?? generateToneScale(brand.tokens.primary)).map(([step, color]) => <div className="rounded-lg border border-[#ddd6eb] p-2" key={step}><span className="block h-8 rounded-md" style={{ backgroundColor: color }} /><p className="mt-1 text-[10px] font-bold text-[#554d62]">{step}</p><p className="font-mono text-[10px] text-[#7a7289]">{color}</p></div>)}</div></TokenSection>
              ...
              <section className="rounded-2xl border border-[#d8d4e7] bg-white/75 p-4 shadow-[0_10px_24px_rgba(47,35,92,0.04)]">
                ...
                {override ? <><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><ColorTokenField id="override-accent" label="Accent override" onChange={(value) => patchOverride({ accent: value })} value={override.accent || brand.tokens.primary} /><ColorTokenField id="override-surface" label="Surface override" onChange={(value) => patchOverride({ surface: value })} value={override.surface || brand.tokens.surface} /><TextField id="override-radius" label="Radius override" onChange={(value) => patchOverride({ radius: value })} value={override.radius || brand.geometry.controlRadius} /><SelectField id="override-density" label="Density" onChange={(value) => patchOverride({ density: value })} options={overrideDensityOptions} value={override.density || "comfortable"} /></div><div className="mt-3 rounded-xl border border-[#e0dbed] bg-[#fbfaff] p-3"><p className="text-xs font-bold uppercase tracking-[0.12em] text-[#665c78]">State matrix · hover / focus / pressed / disabled</p><div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><ColorTokenField id="override-hover-accent" label="Hover accent" onChange={(value) => patchOverride({ states: { ...override.states, hoverAccent: value } })} value={override.states?.hoverAccent || brand.tokens.primaryHover} /><ColorTokenField id="override-focus" label="Focus ring" onChange={(value) => patchOverride({ states: { ...override.states, focusRing: value } })} value={override.states?.focusRing || brand.tokens.focusRing} /><TextField id="override-hover-lift" label="Hover lift" onChange={(value) => patchOverride({ states: { ...override.states, hoverLift: value } })} value={override.states?.hoverLift || brand.motion.hoverLift} /><TextField id="override-pressed-scale" label="Pressed scale" onChange={(value) => patchOverride({ states: { ...override.states, pressedScale: value } })} value={override.states?.pressedScale || brand.motion.pressScale} /><TextField id="override-disabled-opacity" label="Disabled opacity" onChange={(value) => patchOverride({ states: { ...override.states, disabledOpacity: value } })} value={override.states?.disabledOpacity || "0.45"} /></div></div></> : null}
              </section>
            </div>
          ) : null}
