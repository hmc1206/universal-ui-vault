...
/** Karrot의 빠르고 직접적인 feedback tone을 가진 상태 안내 component입니다. */
export function KarrotToast({ className, description, duration = 0, message, onClose, onDismiss, open = true, position, placement, status, title, variant, visible = true, ...props }: KarrotToastProps) {
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1"><p className="text-sm font-bold">{copy}</p>{description ? <p className="mt-1 text-xs leading-5 text-[#868b94]">{description}</p> : null}{status || variant ? <p className="mt-2 text-[10px] font-bold tracking-[0.15em] text-[#ff6f0f]">{status ?? variant}</p> : null}</div>
