...
export function LikelionAvatar({ alt, className, name = "Alex Kim", showStatus = false, size = "md", src, status = "active", tone = "dark", ...props }: LikelionAvatarProps) {
  ...
  return <div {...props} className={joinClasses("relative isolate inline-flex shrink-0 rounded-full bg-white/48 p-0.5 backdrop-blur-md shadow-[0_8px_18px_rgba(15,23,42,0.12)] motion-reduce:transform-none motion-reduce:transition-none contrast-more:outline contrast-more:outline-2 contrast-more:outline-current focus-within:outline focus-within:outline-2 focus-within:outline-current font-mono", sizes[size], className)}>{src ? <img alt={alt ?? name} className="h-full w-full rounded-full border-2 border-white object-cover" src={src} /> : <span aria-label={alt ?? name} className={joinClasses("inline-flex h-full w-full items-center justify-center rounded-full border-2 border-[#f4c6a7] font-black", tone === "light" ? "bg-white text-[#222222]" : "bg-[#ff6000] text-white")} role="img">{initials(name)}</span>}{showStatus ? <span aria-label={status} className={joinClasses("absolute bottom-0 right-0 h-4 w-4 rounded-full border-2 border-white", statusClass)} /> : null}</div>;
}
