...
export function GoodChoiceToast({ className, description, duration = 0, message, onClose, onDismiss, open = true, position, placement, status, title, variant, visible = true, ...props }: GoodChoiceToastProps) {
  ...
  return (
    <div
      {...props}
      aria-live="polite"
      className={joinClasses("fixed z-50 flex w-[min(calc(100vw-2rem),380px)] items-start gap-3 rounded-2xl border border-[#ffd2cf] bg-white p-4 text-[#222222] font-sans transition-all duration-300 ease-out", location.includes("top") ? "top-4" : "bottom-4", location.includes("right") ? "right-4" : "left-4", className)}
      role="status"
    >
