...
/** 여기어때의 선택 구조와 focus rhythm을 담은 native-accessible select입니다. */
export function GoodChoiceSelect({ className, description, disabled = false, error, errorMessage, label, onChange, options, placeholder = "옵션을 선택하세요", required = false, value = "", ...props }: GoodChoiceSelectProps) {
  const id = useId();
  ...
  function handleChange(event: ChangeEvent<HTMLSelectElement>) {
    ...
    if (onChange) (onChange as (value: string, option: GoodChoiceSelectOption) => void)(event.target.value, option);
  }
  return (
