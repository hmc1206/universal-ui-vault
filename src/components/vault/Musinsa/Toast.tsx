...
export function MusinsaToast({ className, description, duration = 0, message, onClose, onDismiss, open = true, position, placement, status, title, variant, visible = true, ...props }: MusinsaToastProps) {
  ...
  return (
    <div
      {...props}
      aria-live="polite"
      className={joinClasses("fixed z-50 overflow-hidden border border-white/45 bg-white/76 backdrop-blur-xl shadow-[0_18px_44px_rgba(15,23,42,0.18)] motion-reduce:transform-none motion-reduce:transition-none contrast-more:outline contrast-more:outline-2 contrast-more:outline-current focus-within:outline focus-within:outline-2 focus-within:outline-current flex w-[min(calc(100vw-2rem),380px)] items-start gap-3 rounded-none border border-[#111111] bg-white p-4 text-[#000000] font-sans font-black transition-all duration-150 ease-out", location.includes("top") ? "top-4" : "bottom-4", location.includes("right") ? "right-4" : "left-4", className)}
      role="status"
    >
