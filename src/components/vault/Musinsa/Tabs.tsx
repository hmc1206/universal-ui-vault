...
export function MusinsaTabs({ ariaLabel = "콘텐츠 탭", className, layout = "hug", onChange, tabs, value }: MusinsaTabsProps) {
  return <div aria-label={ariaLabel} className={joinClasses("flex gap-1 overflow-x-auto p-1 rounded-none border border-[#111111] bg-white font-sans font-black", layout === "fill" && "w-full", className)} role="tablist">{tabs.map((tab) => { const active = tab.value === value; return <button aria-selected={active} className={joinClasses("min-h-10 shrink-0 px-4 text-sm font-bold rounded-none transition-all duration-150 ease-out", layout === "fill" && "flex-1", active ? "bg-[#000000] text-white shadow-[0_4px_12px_rgba(0,0,0,0.12)]" : "text-[#666666] hover:bg-[#ffffff]")} disabled={tab.disabled} key={tab.value} onClick={() => onChange(tab.value)} role="tab" type="button">{tab.label}</button>; })}</div>;
}
