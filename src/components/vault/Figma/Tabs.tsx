...
export function FigmaTabs({ ariaLabel = "콘텐츠 탭", className, layout = "hug", onChange, tabs, value }: FigmaTabsProps) {
  return <div aria-label={ariaLabel} className={joinClasses("grid grid-flow-col auto-cols-fr gap-1 overflow-x-auto border border-white/45 bg-white/64 p-1 backdrop-blur-xl shadow-[0_10px_26px_rgba(15,23,42,0.10)] motion-reduce:transform-none motion-reduce:transition-none contrast-more:outline contrast-more:outline-2 contrast-more:outline-current focus-within:outline focus-within:outline-2 focus-within:outline-current p-1 rounded-sm border border-[#4d4d4d] bg-white font-['figmaSans']", layout === "fill" && "w-full", className)} role="tablist">{tabs.map((tab) => { const active = tab.value === value; return <button aria-selected={active} className={joinClasses("min-h-10 min-w-max px-4 text-sm font-bold rounded-sm transition-all duration-150 ease-out", layout === "fill" && "flex-1", active ? "bg-[#2c2c2c] text-white shadow-[0_4px_12px_rgba(0,0,0,0.12)]" : "text-[#b8b8b8] hover:bg-[#2c2c2c]")} disabled={tab.disabled} key={tab.value} onClick={() => onChange(tab.value)} role="tab" type="button">{tab.label}</button>; })}</div>;
}
