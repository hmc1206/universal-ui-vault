...
/** 29CM의 집중된 결정을 위한 overlay dialog입니다. */
export function TwentyNineCmModal({ actions, children, className, closeOnBackdrop = true, description, dismissible = true, footer, onClose, open, showCloseButton = true, title }: TwentyNineCmModalProps) {
  const titleId = useId();
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 font-sans" role="presentation">
      ...
      <section aria-labelledby={titleId} aria-modal="true" className={joinClasses("relative z-10 w-full border border-white/50 bg-white/78 backdrop-blur-2xl shadow-[0_28px_76px_rgba(15,23,42,0.24)] motion-reduce:transform-none motion-reduce:transition-none contrast-more:outline contrast-more:outline-2 contrast-more:outline-current focus-within:outline focus-within:outline-2 focus-within:outline-current max-w-md overflow-hidden rounded-none border border-[#d8d8d8] bg-white shadow-[0_24px_60px_rgba(0,0,0,0.2)]", className)} role="dialog">
        ...
        {footer || actions ? <div className="flex flex-wrap justify-end gap-2 border-t border-[#d8d8d8] p-4">{footer ?? actions?.map((action, index) => <button className={joinClasses("min-h-10 px-4 text-sm font-bold", index === actions.length - 1 ? "rounded-none border border-black bg-black text-white hover:bg-[#ff4800] active:scale-[0.98]" : "rounded-none border border-[#d8d8d8] bg-white")} key={`${action.label}-${index}`} onClick={action.onClick} type="button">{action.label}</button>)}</div> : null}
      </section>
    </div>
  );
}
